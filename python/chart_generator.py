...
def line_chart_from_df(df, symbol, company, start_date, end_date, *, save_data=False, mock=False):
    ...
    ax.set_title(f"{company} Stock Value From {start_date} to {end_date}{' (Mock Data)' if mock else ''}")
    ax.set_xlabel("Date")
    ax.set_ylabel("Stock Price ($)")
    ax.grid()
    
    # save chart
    plt.savefig(f"../output/{'mock_' if mock else ''}change_in_stock_{symbol}_{start_date}_to_{end_date}.png")
    
    plt.show()

    # save the data if the user requests it
    if save_data:
        df.to_csv(f"../output/{'mock_' if mock else ''}change_in_stock_{symbol}_{start_date}_to_{end_date}.csv", index=False)


def pie_chart_from_df(df, date, *, save_data=False, mock=False):
    ...
    plt.title(f"% of Market by Sector{' (Mock Data)' if mock else ''}")

    # save chart
    plt.savefig(f"../output/{'mock_' if mock else ''}sector_comparison_{date}.png")
    ...
    if save_data:
        df.to_csv(f"../output/{'mock_' if mock else ''}sector_comparison_{date}.csv", index=False)


def bar_chart_from_df(df, currency, date, *, save_data=False, mock=False):
    ...
    # save data
    if save_data:
        df.to_csv(f"../output/{'mock_' if mock else ''}currency_exchange_{date}.csv", index=False)
...
def change_in_stock_chart(company, start_date, end_date, *, save_data=False):
    ...
    while current_date <= end_date:
        ...
        try:
            ...
            print(f"{current_date}: {response['close']}")
        except KeyError:
            pass
    ...
    line_chart_from_df(df, symbol, company, start_date, end_date, save_data=save_data, mock=False)
...
def sector_comparison_chart(date, *, save_data=False):
    ...
    # loop over every company in S&P 500, make call to api for closing price on the given date, and add it to data list
    ...
    pie_chart_from_df(df, date, save_data=save_data, mock=False)
...
def currency_exchange_chart(currency, all_currencies, date, *, save_data=False):
    ...
    bar_chart_from_df(df, currency, date, save_data=save_data, mock=False)